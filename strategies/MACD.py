...
class MACD(object):
    ...
    def _signal(self):
        ...
        """
        raise NotImplementedError("Should implement _signal()")
...
class MovingAverageConvergenceDivergence(MACD):
    ...
    def process_symbol(self, symbol, summarize=True, plot=True):
        try:
            ...
            for row in self.data.itertuples():
                ...
                macd_ema = row.__getattribute__(self.macd_ema)
                if prev_pos == 1:
                    if row_macd < macd_ema < 0:
                        pos = 0
    ...
    def _signal(self):
        last_macd = self.data.MACD.values[-1]
        last_macd_ema = self.data[self.macd_ema].values[-1]
...
class FastMovingAverageConvergenceDivergence(MACD):
    ...
    def _signal(self):
        ...
        if (self.AnnualizedStrategyLogRet > self.AnnualizedMarketLogRet and
            self.AnnualizedStrategyLogRet > self.benchmark):
            if last_macd > last_macd_ema:
                return 'BUY'
            elif last_macd < last_macd_ema:
                return 'SELL'
            else:
                return 'NONE'
        else:
...
if __name__ == '__main__':
    ...
    print macd.process_symbol('LLNW')
    fmacd = FastMovingAverageConvergenceDivergence((5, 25, 25), '2017-01-01', data_handle)
    print fmacd.process_symbol('LLNW')
